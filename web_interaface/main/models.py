...
class Problem(models.Model):
    ...
    def no_of_test_cases(self):
        return len(self.testcase_set.all())
    def status(self):
...
class Submission(models.Model):
    ...
    def task_detail(self):                
        ...
        for case in self.problem.testcase_set.all():
            tests.append({'input':case.input_file.read(), 'output':case.output_file.read()})
            marks.append(case.marks)
        return [self.problem, self.language, self.program.read(),self.filename,tests, marks]
